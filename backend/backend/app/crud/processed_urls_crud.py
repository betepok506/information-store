...
from backend.app.schemas.processed_urls_schema import IProcessedUrlsCreate, IProcessedUrlsUpdate
...
class CRUDProcessedUrls(CRUDBase[ProcessedUrls, IProcessedUrlsCreate, IProcessedUrlsUpdate]):
    async def get_by_hash(
        self, *, hash: str, db_session: AsyncSession | None = None
    ) -> ProcessedUrls | None:
        db_session = db_session or super().get_db().session
        processed_url = await db_session.exec(select(ProcessedUrls).where(ProcessedUrls.hash == hash))
